...
class AemHeadlessClient {
   ...
   * @param {*} preview whether to use preview mode (different service URL)
   */
  constructor({ serviceURL, preview = false }) {
    // Use different service URL based on preview mode
    //this.aemHost = preview ? process.env.NEXT_PUBLIC_AEM_HOST_AUTHOR : serviceURL;
  ...
  async getData(path, cfPath = '', queryVariables = {}, queryParam) {
 ...
 */
const defaultClient = new AemHeadlessClient({ serviceURL: process.env.NEXT_PUBLIC_AEM_HOST });

/**
 * Create a new AEM Headless client instance with preview mode
 * @param {boolean} preview whether to use preview mode
 * @returns {AemHeadlessClient} AEM headless client instance
 */
export function createAemHeadlessClient(preview = false) {
  return new AemHeadlessClient({ 
    serviceURL: process.env.NEXT_PUBLIC_AEM_HOST_AUTHOR, 
    preview 
  });
}
